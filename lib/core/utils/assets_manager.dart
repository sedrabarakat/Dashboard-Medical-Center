class AssetsManager {
  static const _imagePath = "assets/images/";
  // ignore: unused_field
  static const _animationPath = "assets/animation/";

  /*>>>>>>>>>> SVG <<<<<<<<<<*/
  static const drawerImage = '${_imagePath}vaccination_pic.svg';
  static const loginImage = '${_imagePath}login_image.svg';
  static const loginBackgroundImage = '${_imagePath}login_background_image.svg';
  static const uploadFileImage = '${_imagePath}upload-file.svg';
  static const file = "${_imagePath}file.svg";

  /*>>>>>>>>>> PNG <<<<<<<<<<*/
  static const profileImage = '${_imagePath}avatar_2.png';
  static const defImage = '${_imagePath}user.png';

  /*>>>>>>>>>> animation <<<<<<<<<<*/
  static const loadingPatient = 'assets/animation/LoadingPatient.json';
  static const loadingReception = 'assets/animation/loading.json';

  static const heart = '${_imagePath}heart.png';
  /*>>>>>>>>>> JPG <<<<<<<<<<*/
  static const doctor = '${_imagePath}doctor.jpg';
}
